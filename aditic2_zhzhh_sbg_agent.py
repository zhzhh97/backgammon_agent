...
def move(state, die1, die2):
  ...
  global MIN
  MIN = 1 - w
  p, ans = expectiminimax(state, w, die1, die2, MAX_PLAY, False)
...
def expectiminimax(state, whose_move, die1, die2, play_left, expect):
  global CUTOFFS
  ...
  provisional = 0
  if play_left == 0:
    return useSpecialStaticEval(staticEval(state)), 'P'
  if expect:
    provisional = 0
    dice_list = dices()
    for i in range(len(dice_list)):
      e, move = expectiminimax(state, 1 - whose_move, dice_list[i][0], dice_list[i][1], play_left, False)
      provisional += 1 / 36 * e
      chosen = move
    return provisional, chosen
  else:
    ...
    if succ == []:
      ...
      if whose_move == MIN:
        ...
        for s in succ:
          # STATES_CREATED += 1
          e, move = expectiminimax(s, whose_move, 0, 0, play_left - 1, True)
          provisional = min(provisional, e)
          if provisional == e:
            chosen = moves[succ.index(s)]
        return provisional, chosen
      elif whose_move == MAX:
        provisional = float('-inf')
        for s in succ:
